 ...
 * @returns {PracticeResults}
 */
export function calculatePracticeResults(session) {
  ...
  // Weak areas = any subject or system below 60% with ≥2 questions
  const weakAreas = [
  ...
  // Medica Score — internal estimate only, not USMLE prediction
  // Factors: accuracy (70%), difficulty weighting (20%), completion (10%)
...
function _recommendation(weakAreas, score, subjectBreakdown, systemBreakdown) {
  if (weakAreas.length > 0) {
    ...
    return `Next session target: ${top.name} — ${top.percentage}% accuracy signals a retrieval gap. Targeted repetition in this area has the highest readiness return.`
  }
  if (score >= 80) {
    return 'Strong session. Advance to NBME Difficult or UWorld Challenge to maintain upward trajectory.'
  }
  if (score >= 65) {
    const lowestSystem = [...systemBreakdown].sort((a, b) => a.percentage - b.percentage)[0]
    if (lowestSystem && lowestSystem.percentage < 75) {
      return `Solid progress. Target ${lowestSystem.name} next — it's the highest-leverage area to push toward mastery.`
    }
    return 'Solid progress. Another block at this difficulty will reinforce retention.'
  }
  return 'Review the explanations for each missed question before your next session. Repeat this configuration until accuracy stabilizes.'
}

/**
 * @typedef {Object} PracticeResults
